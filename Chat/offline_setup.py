...
def draw_menu(stdscr):
    ...
    stdscr.addstr(1, 10, "SETUP(OFFLINE)", curses.A_BOLD)
    
    for idx, item in enumerate(menu_items):
        if idx == current_item:
            stdscr.addstr(3 + idx, 5, f"> {item}: {offline_settings[item]} <", curses.A_REVERSE)
        else:
            stdscr.addstr(3 + idx, 5, f"{item}: {offline_settings[item]}")
    ...
    while True:
        ...
        if key == curses.KEY_UP and current_item > 0:
            ...
            if item == "model":
                ...
                selected_index[item] = (selected_index[item] + step) % len(offline_model_options)
                offline_settings[item] = offline_model_options[selected_index[item]]
            elif item == "simplify":
                step = 1 if key == curses.KEY_RIGHT else -1
                selected_index[item] = (selected_index[item] + step) % len(simplify_options)
                offline_settings[item] = simplify_options[selected_index[item]]
