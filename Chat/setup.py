...
def draw_menu(stdscr):
    ...
    for idx, item in enumerate(menu_items):
        if idx == current_item:
            stdscr.addstr(3 + idx, 5, f"> {item}: {settings[item]} <", curses.A_REVERSE)
        else:
            stdscr.addstr(3 + idx, 5, f"{item}: {settings[item]}")
    ...
    while True:
        ...
        if key == curses.KEY_UP and current_item > 0:
            ...
            if item == "model":
                ...
                selected_index[item] = (selected_index[item] + step) % len(model_options)
                settings[item] = model_options[selected_index[item]]
            elif item == "simplify":
                step = 1 if key == curses.KEY_RIGHT else -1
                selected_index[item] = (selected_index[item] + step) % len(simplify_options)
                settings[item] = simplify_options[selected_index[item]]
